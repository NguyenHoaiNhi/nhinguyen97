...
class GridEYE_Viewer():
    ...
    def get_image(self):
        if self.START == True:
            dst_get_image = np.zeros((113, 113, 3), dtype=np.uint8)
            ...
            for i in range(0, 113):
                for j in range(0, 113):
                    ...
                    BGR_opencv_get.append(RGB_get[2]*255)
                    dst_get_image[i, j, 0] = BGR_opencv_get[0]
                    BGR_opencv_get.append(RGB_get[1]*255)
                    dst_get_image[i, j, 1] = BGR_opencv_get[1]
                    BGR_opencv_get.append(RGB_get[0]*255)
                    dst_get_image[i, j, 2] = BGR_opencv_get[2]
            path = 'D:/Python/gesture_recognition/My_test/test/duy'
